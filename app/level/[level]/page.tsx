...
// Helper to convert slug to display name with proper capitalization
function levelToTitle(level: string): string {
...
export default async function LevelPage({ params }: LevelPageProps) {
  ...
  const { data: courses, error } = await fetchCoursesByLevel(level as 'beginner' | 'intermediate' | 'advanced');
  ...
  return (
    <SubscriberGate>
      {renderLevelPage(level, courses)}
    </SubscriberGate>
  );
}

function renderLevelPage(level: string, courses: any[]) {
    ...
    <div className="min-h-screen bg-[#FBF9F6]">
      ...
      >
        <div className="max-w-7xl mx-auto text-center relative z-10">
          <Badge className={`mb-4 ${levelColor} text-white border-0 shadow-lg`}>
          ...
          <p className="text-lg md:text-xl text-gray-800 max-w-3xl mx-auto">
            {courses.length} {courses.length === 1 ? 'course' : 'courses'} for {title.toLowerCase()} students
          </p>
        </div>
      </div>
...
export async function generateMetadata({ params }: LevelPageProps) {
  ...
  return {
    title: `${title} Courses | The Piped Peony Academy`,
    description: `Browse ${title.toLowerCase()} level courses. Learn buttercream flower piping, cake decorating, and more at your skill level.`,
  };
}
